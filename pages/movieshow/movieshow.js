...
const url_reservation =
  API_URL + "/reservations/reservations-for-authenticated";
...
export async function initMovieShow(match) {
  ...
  shows = await fetch(url + movieId, makeOptions("GET", null, true)).then((r) =>
    handleHttpErrors(r)
  );
  ...
  dateselection.innerHTML =
    ...
    uniqueDates.map((date) => `<option>${date}</option>`).join("");
  ...
  function updateTimeslotSelector(selectedDate) {
    ...
    const filteredShows = shows.filter(
      (show) => show.showingDate === selectedDate
    );
    const uniqueTimeslots = [
      ...new Set(filteredShows.map((show) => show.timeslot)),
    ];
  ...
  document.querySelector("#complete-booking").addEventListener('click', async () => {await makeReservation()})
  document.querySelector("#cancel-booking").addEventListener('click', async () => {await clearbooking()})
}
//functions
function clearseats() {
...
async function makeReservation() {
  ...
  try {
    const res = await fetch(
      url_reservation,
      makeOptions("POST", reservationRequest, true)
    ).then((r) => handleHttpErrors(r));
